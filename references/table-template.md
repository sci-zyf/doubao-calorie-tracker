...
首次使用本技能时，若 `config.json` 不存在或 `initialized=false`，按本模板自动创建三张表。

## 建表顺序（必须按此顺序，lookup 依赖前表存在）
...
4. 从各命令返回值中提取 base_token 和三个 table_id，写入 config.json
5. 询问用户身高、出生日期、性别，写入 config.json 的 profile 并置 `initialized: true`
6. 提示初始化完成

所有 `lark-cli base` 命令必须加 `--as user`；formula/lookup 字段创建必须加 `--i-have-read-guide`。
...
[
  {"type": "datetime", "name": "日期", "style": {"format": "yyyy-MM-dd"}},
  {"type": "number", "name": "基础代谢", "style": {"type": "plain", "precision": 0}, "default_value": 1800},
  {"type": "number", "name": "每日预期摄入", "style": {"type": "plain", "precision": 2}, "default_value": 1600},
  ...
  {"type": "formula", "name": "剩余可摄入", "expression": "[每日预期摄入]-[总热量]"},
  {"type": "formula", "name": "热量缺口", "expression": "[基础代谢]-[总热量]"},
...
- 每日汇总表的餐次热量与每日营养素均为 formula 字段：用 `COUNTIF` 判断当天是否有对应记录，无记录时返回 `0`（而非空字符串，确保后续 `总热量`/`总质量` 等求和字段计算正确），有记录时用 `FILTER+SUM` 聚合；餐次热量额外带餐次条件。日期格式统一使用小写 `yyyy-MM-dd`（大写 `YYYY` 为 ISO 周年，跨年周会导致匹配错误）。
- 公式字段由表内自动计算，写入记录时只写存储字段（日期、基础代谢、每日预期摄入等），不写 formula。
- 初始化完成后，删除本引导过程中可能产生的任何测试数据（删除前向用户确认）。
- **性能注意**：每日汇总表的 formula 字段会对饮食记录表做全表 `FILTER`/`COUNTIF`，每条汇总记录独立计算。当饮食记录超过 3000 条时，打开汇总表或写入新记录后的公式重算可能明显变慢。若遇到性能问题，可考虑：① 将 formula 聚合改为 `lookup + rollup` 字段（需先在饮食记录表建立关联到每日汇总的 link 字段）；② 或在写入饮食记录时由 agent 增量更新汇总表的存储字段，移除 formula 字段。
